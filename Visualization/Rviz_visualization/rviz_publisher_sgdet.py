    ...
    def Process(self, startfrom=0):
        ...
        while not rospy.is_shutdown():
            ...
            if self.pc_loader is not None:
                ...
                publish_point_cloud(self.pcl_pub, point_cloud, 1)
...
if __name__ == "__main__":
    ...
    if DET:
        ...
        annos_path = "/home/zhanghao/code/GitLab/lidar_ped_veh_model/work_dirs/sg_train3/eval_det_multi/inference/"

        annos_names = os.listdir(annos_path)
        annos_names = sorted(annos_names, key=lambda x: float(x[:12]))

        annos_files = [annos_path+ann for ann in annos_names]
        lidar_files = [lidar_path+ann for ann in annos_names]


        lidar_generator = LidarDataLoader(lidar_files, ndim=4)

        annos_generator = Box3dDataLoader(annos_files, filt_thres=0.6, is_track=False, is_waymo=True)

        rp = RvizPublisher("sg_node", 10, lidar_generator, None, annos_generator, field_angle=60)

        rp.Process()
    else:
        lidar_path = "/mnt/data/SGTrain/20220801/lidar/lidar_undist/"
        annos_path = "/mnt/data/SGTrain/20220801/lidar/labelled_lidar/"

        annos_names = os.listdir(annos_path)
        annos_names = sorted(annos_names, key=lambda x: float(x[:12]))

        annos_files = [annos_path+ann for ann in annos_names]
        lidar_files = [(lidar_path+ann).replace(".json", ".bin") for ann in annos_names]


        lidar_generator = LidarDataLoader(lidar_files, ndim=4)

        annos_generator = Box3dDataLoader(annos_files, filt_thres=0.1, is_track=False, is_waymo=True)
